...
// ─── System Roster Upsert (Leaderboard Sync) ────────────────────────────────

/**
 * Find an existing system-generated roster for a given trial.
 * System rosters are identified by `author_id` (e.g., 'system:eso-1').
 */
...
/**
 * Create or update a system-generated roster.
 * If `existingId` is provided, updates that roster; otherwise creates a new one.
 */
export async function upsertSystemRoster(
  ...
  if (data.existingId) {
    ...
    await db
      .prepare(
        "UPDATE rosters SET title = ?, description = ?, roster_data = ?, updated_at = datetime('now') WHERE id = ? AND author_id = ?",
      )
      .bind(data.title, data.description, data.rosterData, data.existingId, data.authorId)
      .run();
  ...
  await db
    .prepare(
      'INSERT INTO rosters (id, author_id, author_name, title, description, trial_id, roster_data, is_anonymous) VALUES (?, ?, ?, ?, ?, ?, ?, 0)',
    )
    .bind(
      data.newId,
      data.authorId,
  ...
  if (data.tags.length > 0) {
    await insertTags(db, data.newId, data.tags);
  }
...
export async function upsertGuildConfig(
  ...
  await db
    .prepare(
      `INSERT INTO guild_configs (guild_id, guild_name, guild_icon, roster_channel_id, allowed_role_ids, configured_by, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
       ON CONFLICT (guild_id) DO UPDATE SET
         guild_name        = excluded.guild_name,
         guild_icon        = excluded.guild_icon,
         roster_channel_id = excluded.roster_channel_id,
...
export async function deleteGuildConfig(
  ...
  await db.prepare('DELETE FROM guild_configs WHERE guild_id = ?').bind(guildId).run();
}

export async function getAllGuildConfigs(db: D1Database): Promise<GuildConfigRow[]> {
  const result = await db.prepare('SELECT * FROM guild_configs').all<GuildConfigRow>();
...
export async function linkDiscord(
  ...
  await db
    .prepare(
      `INSERT INTO user_profiles (author_id, author_name, bio, discord_id, discord_username, discord_avatar, discord_linked_at, updated_at)
    ...
    .bind(authorId, authorName, discordId, discordUsername, discordAvatar)
    .run();
}

export async function unlinkDiscord(db: D1Database, authorId: string): Promise<void> {
  await db
    .prepare(
      `UPDATE user_profiles
       SET discord_id = NULL, discord_username = NULL, discord_avatar = NULL, discord_linked_at = NULL, updated_at = datetime('now')
       WHERE author_id = ?`,
    )
    .bind(authorId)
    .run();
...
export async function getDiscordId(db: D1Database, authorId: string): Promise<string | null> {
  const row = await db
    .prepare('SELECT discord_id FROM user_profiles WHERE author_id = ?')
    .bind(authorId)
...
export async function getDiscordLink(
  db: D1Database,
  authorId: string,
): Promise<{ discord_id: string; discord_username: string; discord_avatar: string | null } | null> {
  const row = await db
    .prepare('SELECT discord_id, discord_username, discord_avatar FROM user_profiles WHERE author_id = ?')
    .bind(authorId)
    .first<{ discord_id: string | null; discord_username: string | null; discord_avatar: string | null }>();
  if (!row?.discord_id) return null;
  return { discord_id: row.discord_id, discord_username: row.discord_username ?? '', discord_avatar: row.discord_avatar };
}
