...
> **Before creating any PR**: run `npm run validate` AND `npm run test:ci` — both must pass with zero errors/warnings. Do not open a PR or move a ticket to "In Review" until they do.
...
[the GPU queue log](documentation/architecture/fight-replay-npc-gpu-queue-log.md) first. GPU
