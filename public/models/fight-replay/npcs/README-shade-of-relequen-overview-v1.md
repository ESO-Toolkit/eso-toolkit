# Shade of Relequen overview replay prototype (v1)
...
- Reference page: <https://esomodelviewer.com/characters/post/235-shade-of-relequen>
- Encounter: Shade of Relequen, Cloudrest trial — male Welkynar in a floor-length robe over a light cuirass
...
- Registered closeups: **front and back torso only**. Every head and leg closeup was rejected on the
  overlay. Robe cone with no visible leg geometry. Back-plate shoulder detection misfired on the feather ornaments and was hand-set.
- Built through `tools/fight-replay-models/build-npc-asset.py` from `npcs/shade-of-relequen.json`, which records
  the accepted and rejected plates with their measured errors.
- Prepared asset: `shade-of-relequen-overview-v1.glb`; one mesh, one material, one draw call,
  45,000 triangles, 32,337 vertices, 1024x1024 JPEG q92 base-colour texture
  (4:4:4, no chroma subsampling), 1,741,760 bytes.
- Prepared bounds: model height 1.9935 units, minimum Y exactly 0.0, centred on X and Z.
