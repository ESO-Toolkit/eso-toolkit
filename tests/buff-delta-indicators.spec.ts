...
const TEST_PLAYER_NAME = process.env.E2E_PLAYER_NAME ?? 'SamplePlayer5';
...
test.describe('Buff/Debuff Delta Indicators', () => {
  ...
  test('should display neutral indicator for delta = 0%', async ({ page }) => {
    ...
    // Select the configured sample player who has the delta = 0% case
    ...
    if (await playerSelector.isVisible()) {
      ...
      await page.getByText(TEST_PLAYER_NAME).click();
    }
  ...
  test('should display up arrow for large positive deltas (≥ 2%)', async ({ page }) => {
    ...
    const hasUpArrow = await sunderedEntry
      .locator('svg, img[src*="arrow"], [data-testid*="arrow"]')
      .count();
  ...
  test('should display down arrow for large negative deltas (≤ -2%)', async ({ page }) => {
    ...
    const hasDownArrow = await minorBerserkEntry
      .locator('svg, img[src*="arrow"], [data-testid*="arrow"]')
      .count();
  ...
  test('should display indicators for all deltas without minimum threshold', async ({ page }) => {
    ...
    const debuffsWithIndicators = page
      .locator('[role="listitem"]')
      .filter({
        has: page.locator('text=≈'),
      })
  ...
  test('should maintain indicator visibility when switching between stack levels', async ({
    page,
  }) => {
    await page.goto(`/report/${TEST_REPORT_CODE}/fight/${TEST_FIGHT_ID}/insights`);
    await page.waitForLoadState('networkidle');
