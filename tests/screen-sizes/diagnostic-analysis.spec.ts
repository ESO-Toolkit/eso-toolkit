...
test.describe('Diagnostic Test - Content Detection Analysis', () => {
  test('analyze what content detection is actually finding', async ({ page }) => {
    ...
    const contentAnalysis = await page.evaluate(() => {
      ...
      const hasPlayerData = document.querySelector(
        '[data-testid*="player-row"], .player-card, [data-testid="players-table"] tbody tr:not(.skeleton)',
      );
      const hasInsightsData = document.querySelector(
        '[data-testid*="chart"], [data-testid*="insights-"], .chart-container canvas, [data-testid="insights-content"] *:not(.skeleton)',
      );
      const hasTableData = document.querySelector(
        'table tbody tr:not(.skeleton):not([data-testid*="skeleton"]), .data-row, [data-loaded="true"]',
      );

      // Check for loading states
      const hasActiveLoading = document.querySelector(
        '[data-testid*="loading"]:not([style*="display: none"]), .loading-spinner:not([style*="display: none"])',
      );
      ...
      return {
        ...
        allTableRows,
        allDivs,
        playerDataElement: hasPlayerData
          ? hasPlayerData.tagName + '.' + hasPlayerData.className
          : null,
        tableDataElement: hasTableData ? hasTableData.tagName + '.' + hasTableData.className : null,
        insightsDataElement: hasInsightsData
          ? hasInsightsData.tagName + '.' + hasInsightsData.className
          : null,
      };
    });
    ...
    console.log(
      'hasInsightsData:',
      contentAnalysis.hasInsightsData,
      contentAnalysis.insightsDataElement,
    );
    ...
    const reportTitleCount = await page
      .locator('[data-testid="report-title"], h1, .report-header')
      .count();
    const playerContentCount = await page
      .locator('table tbody tr, .player-card, [data-testid*="player"]')
      .count();
