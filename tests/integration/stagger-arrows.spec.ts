...
const REPORT_CODE = process.env.E2E_STAGGER_REPORT_CODE ?? 'F4f2bMwWtgVKxjB9';
const FIGHT_ID = process.env.E2E_STAGGER_FIGHT_ID ?? '5';

test.describe('Stagger Per-Stack Comparison Arrows', () => {
  test.beforeEach(async ({ page }) => {
    // Use the bundled public sample by default; point E2E_STAGGER_REPORT_CODE at
    // a report with stagger data when running this specialized live-data check.
    await page.goto(`http://localhost:3000/reports/${REPORT_CODE}?fight=${FIGHT_ID}`);
  ...
  test('shows per-stack comparison arrows for stagger when player selected', async ({ page }) => {
    ...
    const initialHasUpArrow =
      (await staggerEntry.locator('svg[data-testid="TrendingUpIcon"]').count()) > 0;
    ...
    const newHasDownArrow = await staggerEntry
      .locator('svg[data-testid="TrendingDownIcon"]')
      .count();
    ...
    console.log(
      `Arrow direction: ${initialHasUpArrow ? 'UP' : 'DOWN'} -> ${newHasUpArrow > 0 ? 'UP' : 'DOWN'}`,
    );
  });
  ...
  test('verifies all three stagger stacks show comparison arrows', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const hasUpArrow = await staggerEntry.locator('svg[data-testid="TrendingUpIcon"]').count();
      const hasDownArrow = await staggerEntry
        .locator('svg[data-testid="TrendingDownIcon"]')
        .count();
      ...
      console.log(
        `Stack ${stackText}: ${percentageText} - Arrow: ${hasUpArrow > 0 ? 'UP' : 'DOWN'}`,
      );
  ...
  test('verifies arrow delta values are calculated correctly', async ({ page }) => {
    ...
    const deltaText = await staggerEntry
      .locator('text=/[+-]\\d+%/')
      .first()
      .textContent()
      .catch(() => null);
