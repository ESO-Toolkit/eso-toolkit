 ...
 *
 * Test Coverage:
 * - Loads the configured sample fight data
...
test.describe('Scribing Detection - Regression Tests', () => {
  const REPORT_ID = process.env.E2E_REPORT_CODE ?? 'F4f2bMwWtgVKxjB9';
  const FIGHT_ID = '11';
  ...
  test('should detect Player 1 Shattering Knife as cast in fight', async ({ page }) => {
    ...
    const scribingSection = player1Card
      .locator('[data-testid="scribing-skills"], .scribing-skills, [class*="scribing"]')
      .first();
    ...
    if (await scribingSection.isVisible()) {
      ...
      if (await shatteringKnifeElement.isVisible()) {
        ...
        const talentsSection = player1Card.locator(
          '[data-testid="player-talents"], .talents, [class*="talent"]',
        );
    ...
    const skillTooltips = page.locator(
      '[data-testid="skill-tooltip"], .skill-tooltip, [class*="tooltip"]',
    );
