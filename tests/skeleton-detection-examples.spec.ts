...
test.describe('Skeleton Detection Examples', () => {
  ...
  test('demonstrates skeleton detection on players panel', async ({ page }) => {
    ...
    await esoPage.goToFightPlayers(TEST_REPORT_CODE, TEST_FIGHT_ID);
  ...
  test('demonstrates waiting for loading sequence', async ({ page }) => {
    ...
    await esoPage.goToFightInsights(TEST_REPORT_CODE, TEST_FIGHT_ID);
  ...
  test('demonstrates specific skeleton type detection', async ({ page }) => {
    ...
    if (hasCalculatorSkeleton) {
      ...
      await esoPage.skeletons.waitForSkeletonTypeToDisappear(SKELETON_SELECTORS.CALCULATOR, {
        timeout: 30000,
      });
    ...
    const stillHasSkeleton = await esoPage.skeletons.hasSkeletonType(SKELETON_SELECTORS.CALCULATOR);
