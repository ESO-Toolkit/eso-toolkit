...
test.describe('Nightly Regression - Interactive Features', () => {
  ...
  test.describe('Fight Replay Functionality', () => {
    test('should load and interact with fight replay', async ({ page }, testInfo) => {
      ...
      // Take screenshot of report page with error handling
      try {
        ...
        console.warn('Screenshot failed but continuing test:', (screenshotError as Error).message);
      }
      ...
      if (hasReplayInterface) {
        ...
        try {
          ...
          console.warn('Playing screenshot failed but continuing test:', (screenshotError as Error).message);
        }
        ...
        if (await timeline.first().isVisible({ timeout: 3000 })) {
          ...
          try {
            ...
            console.warn('Scrubbed screenshot failed but continuing test:', (screenshotError as Error).message);
          }
        }
      }
      ...
      if (hasLiveInterface) {
        ...
        if (await controls.first().isVisible({ timeout: 3000 })) {
          ...
          try {
            ...
            console.warn('Live active screenshot failed but continuing test:', (screenshotError as Error).message);
          }
        }
      }
    });
  ...
  test.describe('Advanced Visualizations', () => {
    test('should test location heatmap visualization', async ({ page }) => {
      ...
      try {
        ...
        console.warn('Heatmap screenshot failed but continuing test:', (screenshotError as Error).message);
      }
    ...
    test('should test rotation analysis visualization', async ({ page }, testInfo) => {
      ...
      try {
        ...
        console.warn('Rotation screenshot failed but continuing test:', (screenshotError as Error).message);
      }
