 ...
 *
 * Permission model:
 *   - If the guild has `allowedRoleIds` configured, the user must have at least one of those roles.
 *   - If no roles are configured, fall back to requiring MANAGE_GUILD permission (admin-only until set up).
 *   - Webhooks from roster-hub-api authenticate via a shared secret instead of Discord tokens.
 */
...
export async function verifyHttpCaller(
  ...
  if (allowedRoles && allowedRoles.length > 0) {
    ...
    try {
      ...
      if (!hasRole) {
        return { authorized: false, userId: me.id, error: 'You do not have the required role to publish rosters in this server.' };
      }
  ...
  const hasManage = (BigInt(guild.permissions) & Permission.MANAGE_GUILD) === Permission.MANAGE_GUILD;
  if (!hasManage) {
    return {
      authorized: false,
      userId: me.id,
      error: 'No allowed roles are configured for this server. Only server admins can publish until a role is set up with /roster config set-role.',
    };
  }
