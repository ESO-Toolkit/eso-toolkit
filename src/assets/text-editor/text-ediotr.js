...
- Target dummies to hone your DPS, healing, and tanking skills.`;
        ...
        const bindEmojiHandler = () => {
            ...
            const openColorPicker = (e) => {
                ...
                if (this.isMobileDevice()) {
                    ...
                    if (native) {
                        try { if (typeof native.showPicker === 'function') { native.showPicker(); return; } } catch (_) {}
                        native.click();
                    }
                    return;
                }
                ...
                if (this.esoPickr && typeof this.esoPickr.show === 'function') {
                    try { this.esoPickr.show(); return; } catch (_) {}
                }
                this.createESOFallbackPicker();
                const fb = document.getElementById('eso-fallback-color-picker');
                if (fb) {
                    try { if (typeof fb.showPicker === 'function') { fb.showPicker(); return; } } catch (_) {}
                    fb.click();
                }
            };
        ...
        if (!bindEmojiHandler()) {
            ...
            const mo = new MutationObserver(() => {
                if (bindEmojiHandler()) { mo.disconnect(); }
            });
        ...
        if (emojiBtn) {
            ...
            if (isIOS) {
                ...
                const openNativePicker = (e) => {
                    if (e) { e.stopPropagation(); /* no preventDefault to keep browsers happy */ }
                    try {
    ...
    createESOFallbackPicker() {
        ...
        const openNativePicker = (e) => {
            if (e) { /* avoid preventDefault to satisfy some mobile browsers */ e.stopPropagation(); }
            try {
    ...
    applyColorToSelection(colorHex) {
        ...
        if (match) {
            ...
            const expandedSelection = this.smartExpandSelection(input.value, selection.start, selection.end);
    ...
    smartExpandSelection(text, start, end) {
        ...
        const hasPartialStart = selectedText.includes('|c') && !selectedText.match(/^\|c[0-9A-Fa-f]{6}/);
        const hasPartialEnd = selectedText.includes('|r') && !selectedText.endsWith('|r');
        const hasPartialTags = hasPartialStart || hasPartialEnd || selectedText.match(/\|c[0-9A-Fa-f]{0,5}$/) || selectedText.match(/^\|[^cr]/);
        ...
        const surroundingContext = text.substring(Math.max(0, start - 10), Math.min(text.length, end + 10));
    ...
    findEnclosingColorBlock(text, start, end) {
        ...
        for (const block of colorBlocks) {
            ...
            const selectionInsideBlock = (start >= blockStart && start < blockEnd) || 
    ...
    updatePreview() {
        ...
        if (!text.trim()) {
            preview.innerHTML = '<span style="color: #888; font-style: italic;">Your formatted text will appear here...</span>';
            return;
        }
