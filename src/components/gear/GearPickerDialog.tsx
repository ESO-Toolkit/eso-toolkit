/**
 * GearPickerDialog — set-aware gear selection modal (shared core).
 *
 * Extracted from the build editor into a neutral location (src/components/gear)
 * so any surface — the Build Editor's EquipmentPicker, the Loadout Manager's
 * gear editor — can reuse it. It is fully presentational and keyed on item ids:
 * it takes `targetSlot` + `currentItemId` and calls `onSelect(itemId)`, with no
 * Redux or feature coupling, so it cannot create a feature import cycle.
 *
 * A categorized, searchable gear browser matching the glass-morphism design of
 * FoodPicker / PotionPicker.
 *
...
import type { GearSetType } from '@features/build-editor/data/gearSetRegistry';
import { getLockedArmorWeight } from '@features/build-editor/data/setArmorWeights';
