...
describe('DynamicMetaTags', () => {
  ...
  it('should add Open Graph meta tags', () => {
    ...
    expect(document.querySelector('meta[property="og:title"]')?.getAttribute('content')).toBe('Cloudrest +3');
    expect(document.querySelector('meta[property="og:description"]')?.getAttribute('content')).toBe('Amazing DPS performance');
    expect(document.querySelector('meta[property="og:image"]')?.getAttribute('content')).toBe('https://example.com/preview.png');
    expect(document.querySelector('meta[property="og:url"]')?.getAttribute('content')).toBe('https://example.com/report/1');
  ...
  it('should add Twitter Card meta tags', () => {
    ...
    expect(document.querySelector('meta[name="twitter:title"]')?.getAttribute('content')).toBe('ESO Analysis');
    expect(document.querySelector('meta[name="twitter:description"]')?.getAttribute('content')).toBe('Combat log insights');
    expect(document.querySelector('meta[name="twitter:image"]')?.getAttribute('content')).toBe('https://example.com/card.png');
  });
});
...
describe('generatePlayerMetaTags', () => {
  it('should generate player-specific meta tags', () => {
    const result = generatePlayerMetaTags('ABC123', 'TestPlayer', 'Dragonknight', 85000, 'Sunspire');
