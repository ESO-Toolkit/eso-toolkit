/**
 * Skills Section — horizontal ESO-style skill bars with organized skill picker.
 *
 * Two action bars (front/back) with 5 ability slots + 1 ultimate each.
 * Click a slot to open a categorized skill picker organized by skill line,
 * with morph grouping, class-colored headers, and search fallback.
 */
...
const SkillPickerDialog: React.FC<SkillPickerDialogProps> = ({
  ...
  // Group line metadata by tab category
  const linesByTab = useMemo(
    () => PICKER_TABS.map((tab) => lineIndex.filter((l) => l.broadCategory === tab.category)),
    [lineIndex],
  );

  // Class lines grouped by ESO class (proper order from ESO_CLASSES)
  ...
  // Map selected ClassSkillLineIds → skill line display names (e.g. 'Ardent Flame')
  ...
  // "My Lines" filter — defaults on when the user has class lines selected
  const [myBuildOnly, setMyBuildOnly] = useState(hasSelectedLines);

  // Reset search and filter state whenever the dialog opens
  useEffect(() => {
    if (open) {
      setSearch('');
  ...
  // When filtering, narrow class tab to only the 3 chosen lines
        ...
        {isSearching ? (
          ...
          <>
            ...
            >
              ...
              {/* "My Lines" filter toggle — only on Class tab when skill lines are selected */}
...
const SkillBar: React.FC<SkillBarProps> = ({ label, bar, onOpenPicker, onRemove }) => {
  const isDark = useTheme().palette.mode === 'dark';
...
export const SkillsSection: React.FC = () => {
  ...
  // Lifted picker state — one shared dialog across all 12 slots
