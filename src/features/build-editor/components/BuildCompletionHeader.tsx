...
export const BuildCompletionHeader: React.FC = () => {
  ...
  const [mobileDescExpanded, setMobileDescExpanded] = React.useState(false);
  ...
  return (
    ...
    >
      {/* ── Row 1 (mobile) / Left section (desktop): Build identity ── */}
      <Box
        sx={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        ...
        >
          <input
            ...
            onFocus={() => isMobile && setMobileDescExpanded(true)}
            maxLength={80}
            ...
            style={{
              ...
              padding: isMobile ? (mobileDescExpanded ? '8px 12px 0' : '7px 12px') : '8px 12px 0',
              boxSizing: 'border-box',
            }}
          />
          {/* Description: always visible on desktop, expandable on mobile */}
          {(!isMobile || mobileDescExpanded) && (
            <input
              ...
              onBlur={() => isMobile && !build.shortDescription && setMobileDescExpanded(false)}
              maxLength={140}
      ...
      {/* ── Row 2 (mobile) / Right section (desktop): Actions ── */}
      <Box
        sx={{
          display: 'flex',
          alignItems: 'center',
          ...
          ml: { md: 'auto' },
          flexShrink: 0,
          // On mobile: stretch to full width, justify actions
      ...
      >
              ...
              <Tooltip title={`Apply changes to ${slotLabel} in "${rosterName ?? 'Roster'}"`}>
                <Button
                  ...
                  sx={{
                    ...pillBase,
                    background:
                      'linear-gradient(135deg, rgba(139,92,246,0.9), rgba(109,40,217,0.8))',
        ...
        >
          ...
          >
            ...
            {/* Get Link (guest only) — hidden on mobile, available in overflow */}
            {!isLoggedIn && !isMobile && (
              <>
            ...
            {/* Roster — hidden on mobile, available in overflow */}
            {!isMobile && (
              <Tooltip
                title={
                  savedRostersCount === 0
          ...
          {/* ── Overflow menu — visible on medium/mobile viewports ── */}
          {isMedium && (
            <>
              ...
              >
                {/* Mobile-only actions: Get Link + Roster */}
                {isMobile && !isLoggedIn && (
                  <MuiMenuItem
                    onClick={() => {
                      setMoreAnchor(null);
                  ...
                  >
                    ...
                    <ListItemText>
                      {isCreatingLink ? 'Creating link\u2026' : 'Get Shareable Link'}
                    </ListItemText>
                  </MuiMenuItem>
                )}
                {isMobile && (
                  ...
                  >
                    ...
                    <ListItemText>Add to Roster</ListItemText>
                  </MuiMenuItem>
                )}
                {isMobile && <Divider />}
                <MuiMenuItem
