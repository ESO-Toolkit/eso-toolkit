...
interface DiscordPublishDialogProps {
  open: boolean;
...
export const DiscordPublishDialog: React.FC<DiscordPublishDialogProps> = ({
  open,
  roster,
  onClose,
  onSuccess,
  ...
  // Reset state when dialog opens with a new roster
  React.useEffect(() => {
    if (open && roster) {
      setChannelName('');
      setError(null);
  ...
  const handlePublish = async (): Promise<void> => {
    if (!roster || !guildId.trim()) return;

    setLoading(true);
    setError(null);

    try {
      const res = await fetch(`${DISCORD_BOT_API_URL}/discord/roster/publish`, {
        method: 'POST',
    ...
    >
      ...
      <DialogContent sx={{ pt: '8px !important' }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          This will create a Discord channel with a rich roster embed featuring role sections,
          build links, and sign-up buttons. The embed auto-updates when you edit the roster.
        </Typography>
        ...
        <Button
          ...
          startIcon={
            loading ? (
              ...
              <img src={discordIcon} alt="" style={{ width: 16, height: 16, filter: 'brightness(10)' }} />
            )
          }
          sx={{
