  ...
  const fetchData = React.useCallback(async (params: FetchReportSummaryParams) => {
    ...
    try {
      ...
      if (totalFights >= 10) {
        ...
        console.log(`📊 Small report detected (${totalFights} fights) - using single query strategy`);
        setProgress({
          current: 2,
          total: 5,
      ...
      const totalEvents = reportEvents.damageEvents.length + 
      ...
      const summaryData: ReportSummaryData = {
        ...
        fights: cleanFights,
        damageBreakdown: {
          totalDamage: reportEvents.damageEvents.reduce((sum: number, event: any) => sum + (event.amount || 0), 0),
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred during optimized fetching';
