...
describe('affix-script-buff-mappings', () => {
  ...
  describe('getAffixScriptByEffectId', () => {
    ...
    it('should return correct mapping for debuff IDs', () => {
      ...
      const vulnerabilityMapping = Object.values(AFFIX_SCRIPT_BUFF_MAPPINGS)
        .find(mapping => mapping.debuffIds.length > 0);
  ...
  describe('getEffectIdsForAffixScript', () => {
    ...
    it('should handle affix scripts with only buffs', () => {
      const buffOnlyScript = Object.entries(AFFIX_SCRIPT_BUFF_MAPPINGS)
        .find(([, mapping]) => mapping.buffIds.length > 0 && mapping.debuffIds.length === 0);
    ...
    it('should handle affix scripts with only debuffs', () => {
      const debuffOnlyScript = Object.entries(AFFIX_SCRIPT_BUFF_MAPPINGS)
        .find(([, mapping]) => mapping.buffIds.length === 0 && mapping.debuffIds.length > 0);
  ...
  describe('isAffixCompatibleWithGrimoire', () => {
    ...
    it('should return false for incompatible combinations', () => {
      ...
      const result = isAffixCompatibleWithGrimoire('savagery-and-prophecy', 'non-existent-grimoire');
