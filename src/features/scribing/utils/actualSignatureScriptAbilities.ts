...
export function getAbilitiesBySignatureScript(signatureScript: string): SignatureScriptAbility[] {
  return SIGNATURE_SCRIPT_ABILITIES.filter(ability => ability.signatureScript === signatureScript);
}
...
export const ACTUAL_SIGNATURE_SCRIPT_PATTERNS = {
  ...
  'class-mastery': {
    ...
    abilityIds: [
      // Necromancer (12)
      217504, 220129, 220523, 220634, 220851, 221148, 221182, 221305, 221389, 221588, 221657, 227112,
      // Dragonknight (12)
      217479, 220137, 220505, 220616, 220821, 221078, 221155, 221276, 221367, 221571, 221643, 227087,
      // Sorcerer (12)
      217500, 220135, 220509, 220620, 220831, 221132, 221166, 221289, 221373, 221573, 221644, 227093,
      // Templar (12)
      217483, 220140, 220524, 220636, 220830, 221131, 221161, 221288, 221372, 221591, 221662, 227092,
      // Nightblade (12)
      217503, 220144, 220510, 220621, 220834, 221136, 221169, 221292, 221376, 221575, 221646, 227101,
      // Warden (12)
      217508, 220130, 220518, 220629, 220839, 221142, 221173, 221297, 221381, 221577, 221648, 227106,
      // Arcanist (12)
      217507, 220147, 220529, 220641, 220853, 221152, 221185, 221307, 221391, 221596, 221667, 227116,
    ],
    examples: ["Necromancer's Knife", "Dragonknight's Vault", "Sorcerer's Soul", "Templar's Burst", "Nightblade's Throw", "Warden's Bond", "Arcanist's Banner"],
  },
};
...
export const ABILITY_ID_TO_SIGNATURE_SCRIPT = new Map(
  SIGNATURE_SCRIPT_ABILITIES
    .filter(ability => ability.signatureScript !== "unknown")
    .map(ability => [ability.id, ability.signatureScript]),
);

/**
 * Creates a mapping of signature script to abilities for analysis
 */
export const SIGNATURE_SCRIPT_TO_ABILITIES = SIGNATURE_SCRIPT_ABILITIES
  .filter(ability => ability.signatureScript !== "unknown")
  .reduce((acc, ability) => {
...
export function getAllSignatureScripts(): string[] {
  return [...new Set(SIGNATURE_SCRIPT_ABILITIES
    .filter(ability => ability.signatureScript !== "unknown")
    .map(ability => ability.signatureScript))];
}

export default SIGNATURE_SCRIPT_ABILITIES;