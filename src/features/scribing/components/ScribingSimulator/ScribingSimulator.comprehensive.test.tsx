...
describe('ScribingSimulator - Comprehensive Coverage Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockCalculateSkill.mockImplementation((grimoireId: string, focusId?: string, signatureId?: string, affixId?: string) => {
      return {
        name: 'Mock Calculated Skill',
        grimoire: grimoireId,
        focus: focusId,
        ...
        properties: {
          ...
          damage: 250,
          damageType: focusId === 'physical-damage' ? 'physical' : focusId === 'magic-damage' ? 'magic' : undefined,
  ...
  describe('Default State and Grimoire Selection', () => {
    it('should have default grimoire selected and show calculated skill', () => {
      ...
      expect(mockCalculateSkill).toHaveBeenCalledWith('trample', 'physical-damage', undefined, undefined);
    });
  ...
  describe('Focus Script Functionality', () => {
    ...
    it('should recalculate skill when focus changes', async () => {
      ...
      await waitFor(() => {
        expect(mockCalculateSkill).toHaveBeenCalledWith('trample', 'magic-damage', undefined, undefined);
      });
    });
  });
  ...
  describe('Signature Script Functionality', () => {
    ...
    it('should recalculate skill when signature script changes', async () => {
      ...
      await waitFor(() => {
        expect(mockCalculateSkill).toHaveBeenCalledWith('trample', 'physical-damage', 'signature-1', undefined);
      });
    });
  });
  ...
  describe('Affix Script Functionality', () => {
    ...
    it('should recalculate skill when affix script changes', async () => {
      ...
      await waitFor(() => {
        expect(mockCalculateSkill).toHaveBeenCalledWith('trample', 'physical-damage', undefined, 'affix-1');
      });
    });
  });
  ...
  describe('Calculated Skill Display', () => {
    it('should display calculated skill properties', () => {
      ...
      expect(screen.getByText('Mock Calculated Skill')).toBeInTheDocument();
      expect(screen.getByText('This is a mock tooltip for the calculated skill')).toBeInTheDocument();
    });
  ...
  describe('Selection Validation and Clearing', () => {
    it('should clear invalid selections when grimoire changes', async () => {
      ...
      await waitFor(() => {
        expect(mockCalculateSkill).toHaveBeenCalledWith('trample', 'physical-damage', 'signature-2', undefined);
      });
