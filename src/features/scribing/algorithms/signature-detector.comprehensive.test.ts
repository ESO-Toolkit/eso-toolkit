...
describe('SignatureScriptDetector - Comprehensive Tests', () => {
  ...
  describe('ensureMapperReady', () => {
    ...
    it('should throw error when mapper not ready', async () => {
      ...
      await expect(detector.detectSignatureScriptFromGrimoire(grimoire, [])).rejects.toThrow(
        'Scribing mapper not initialized',
      );
    });
  ...
  describe('detectSignatureScriptFromGrimoire', () => {
    ...
    it('should detect enhanced ability mapping with correct grimoire context', async () => {
      ...
      const enhancedDetection = result.find(
        (r) => r.detectionMethod === 'direct-ability' && r.signatureScriptKey === 'hunters-snare',
      );
    ...
    it('should use fallback direct ability mapping when enhanced fails', async () => {
      ...
      abilityScribingMapper.getSignatureByAbilityId
        ...
        .mockResolvedValueOnce({
          // Second call (fallback) returns mapping
    ...
    it('should detect signatures using event patterns', async () => {
      ...
      const patternDetection = result.find(
        (r) =>
          r.detectionMethod === 'buff-analysis' && r.signatureScriptKey === 'lingering-torment',
      );
  ...
  describe('detectSignatureScriptsFromGrimoires', () => {
    ...
    it('should process multiple grimoire detections', async () => {
      ...
      const signature1Detections = result.detections.filter(
        (d) => d.signatureScriptKey === 'signature-1',
      );
      const signature2Detections = result.detections.filter(
        (d) => d.signatureScriptKey === 'signature-2',
      );
    ...
    it('should handle processing errors gracefully', async () => {
      ...
      abilityScribingMapper.getSignatureByAbilityId.mockRejectedValue(new Error('Mapper error'));
    ...
    it('should aggregate player and grimoire signature data', async () => {
      ...
      expect(result.grimoireSignatureScripts.get('traveling-knife')?.has('hunters-snare')).toBe(
        true,
      );
    });
  });
  ...
  describe('pattern detection methods', () => {
    ...
    it('should respect pattern timing windows', async () => {
      ...
      const patternDetection = result.find(
        (r) =>
          r.detectionMethod === 'debuff-analysis' && r.signatureScriptKey === 'lingering-torment',
      );
  ...
  describe('getSignatureScriptStatistics', () => {
    it('should calculate statistics from detection result', () => {
      ...
      expect(stats.signatureScriptsByGrimoire.get('traveling-knife')?.has('signature-1')).toBe(
        true,
      );
