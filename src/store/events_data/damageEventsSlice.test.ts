...
import damageEventsReducer, { DamageEventsState, fetchDamageEvents } from './damageEventsSlice';
...
describe('damageEventsSlice pagination hardening', () => {
  ...
  const entry = () => {
    const state = store.getState() as { events: { damage: DamageEventsState } };
    const { key } = resolveCacheKey({ reportCode: 'ABC123', fightId: Number(fight.id) });
    return state.events.damage.entries[key];
  };
  ...
  it('fails closed when pagination exceeds the page cap', async () => {
    ...
    expect(entry()?.error).toBe('Damage event pagination exceeded 2 pages');
    expect(client.query).toHaveBeenCalledTimes(2);
  });

  it('fails closed when pagination exceeds the per-stream event cap', async () => {
    client.query.mockResolvedValueOnce(
      page([damageEvent(1000), damageEvent(1001), damageEvent(1002)], null),
    );

    await store.dispatch(fetchDamageEvents({ reportCode: 'ABC123', fight, client }) as never);

    expect(entry()?.status).toBe('failed');
    expect(entry()?.error).toBe('Damage event pagination exceeded 2 events');
...
const damageEvent = (timestamp: number) => ({
  type: 'damage',
  timestamp,
  sourceID: 1,
  targetID: 2,
