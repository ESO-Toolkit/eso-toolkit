...
export const statusEffectUptimesSlice = createWorkerTaskSlice(
  ...
  (input) => {
    ...
    const hostileBuffIntervalsCount = input.hostileBuffsLookup?.buffIntervals ? Object.keys(input.hostileBuffsLookup.buffIntervals).length : 0;
    const fightStart = input.fightStartTime ?? 0;
    const fightEnd = input.fightEndTime ?? 0;
    const friendlyPlayerIds = input.friendlyPlayerIds?.slice().sort((a, b) => a - b).join(',') ?? '';
