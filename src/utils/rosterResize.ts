...
function resizeTanks(tanks: TankSetup[], newCount: number): TankSetup[] {
  ...
  for (let i = 0; i < newCount; i++) {
    const existing = i < tanks.length ? JSON.parse(JSON.stringify(tanks[i])) : defaultTankSetup(i + 1);
...
function resizeHealers(healers: HealerSetup[], newCount: number): HealerSetup[] {
  ...
  for (let i = 0; i < newCount; i++) {
    const existing = i < healers.length ? JSON.parse(JSON.stringify(healers[i])) : defaultHealerSetup(i + 1);
    existing.slotNumber = i + 1;
    result.push(existing);
  }
  return result;
...
function resizeDPS(dpsSlots: DPSSlot[], newCount: number): DPSSlot[] {
  ...
  for (let i = 0; i < newCount; i++) {
    const existing: DPSSlot =
      i < dpsSlots.length ? JSON.parse(JSON.stringify(dpsSlots[i])) : ({ slotNumber: i + 1 } as DPSSlot);
    existing.slotNumber = i + 1;
    result.push(existing);
  }
  return result;
