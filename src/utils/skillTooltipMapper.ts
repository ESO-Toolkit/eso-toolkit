...
function toStats(node: any): SkillStat[] {
...
export type MapSkillOptions = {
  ...
  node: any; // ability or morph node
  inheritFrom?: any; // parent ability to inherit base stats from
...
export function mapSkillToTooltipProps(opts: MapSkillOptions): SkillTooltipProps {
  ...
  const descriptionNode: React.ReactNode = ((): React.ReactNode => {
    ...
    const pulseInterval = (merged as any)?.pulseInterval as string | undefined;
    const pulseDamage = (merged as any)?.pulseDamage as string | undefined;
    ...
    const passiveBuff = (inheritFrom as any)?.passiveBuff as string | undefined;
...
function findAbilityNodeByName(
  cls: any,
  abilityName: string
): { node: any; parent?: any; skillLineName: string } | null {
  if (!cls || !cls.skillLines) return null;
  const target = (abilityName || '').trim();
  for (const line of Object.values<any>(cls.skillLines)) {
    ...
    for (const cat of categories) {
      ...
      for (const entry of Object.values<any>(collection)) {
        // Check base ability node
        if (entry?.name === target) {
          return { node: entry, skillLineName };
        }
        // Check morphs if present
        const morphs = entry?.morphs || {};
        for (const morph of Object.values<any>(morphs)) {
          if (morph?.name === target) {
            return { node: morph, parent: entry, skillLineName };
          }
        }
      }
    }
