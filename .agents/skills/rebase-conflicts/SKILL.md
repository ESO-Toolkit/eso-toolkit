...
If the target branch is already checked out in another worktree (e.g. `D:\code\eso-log-aggregator-worktrees\ESO-635-feature-name`), **navigate there directly** — do NOT attempt `git checkout` in the current worktree:

```powershell
# Right: go to the worktree where the branch lives (check `git worktree list` for the path)
Set-Location D:\code\eso-log-aggregator-worktrees\ESO-635-feature-name
