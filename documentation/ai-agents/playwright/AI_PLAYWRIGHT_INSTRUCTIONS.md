# AI Agent Instructions for Playwright Visual Testing - ESO Toolkit

## 🎯 Critical Problem Context

ESO Toolkit has **sophisticated data loading states** with multiple skeleton UI components that must completely disappear before taking screenshots. **Mocked data still takes significant time to load** due to complex React rendering, GraphQL client hydration, and Redux state updates.
...
ESO Toolkit has **different types of skeleton components**:
...
**Question Raised:** _"What good is testing if we are being defensive? Shouldn't we lock content into place?"_
