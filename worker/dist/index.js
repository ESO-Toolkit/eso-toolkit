...
// src/config.ts
var CHAT_MODEL = "@cf/qwen/qwen3-30b-a3b-fp8";
var EMBEDDING_MODEL = "@cf/baai/bge-m3";
var VECTORIZE_TOP_K = 5;
var D1_BUILD_STATS_LIMIT = 10;
...
// src/lib/d1-queries.ts
var queryBuildStats = /* @__PURE__ */ __name(async (db, intent) => {
  const conditions = [];
  const params = [];
...
var queryKnowledgeDocsById = /* @__PURE__ */ __name(async (db, vectorizeIds) => {
  ...
  const result = await db.prepare(sql).bind(...vectorizeIds).all();
  return result.results;
}, "queryKnowledgeDocsById");
var upsertBuildStats = /* @__PURE__ */ __name(async (db, rows) => {
  let inserted = 0;
...
var matchPatterns = /* @__PURE__ */ __name((text, patterns) => {
  const matches = [];
...
var extractIntent = /* @__PURE__ */ __name((message) => {
...
var extractKeywords = /* @__PURE__ */ __name((text) => {
  const gameTerms = [
    "parse",
    "trial",
    "arena",
...
var buildSystemPrompt = /* @__PURE__ */ __name((buildStats, knowledgeDocs) => {
...
var SYSTEM_PREAMBLE = `You are the ESO Toolkit AI assistant, an expert on Elder Scrolls Online combat builds, gear, and optimization. You help players understand weapon traits, enchants, gear sets, and build strategies using real data from ESO Logs parses.

You have access to two data sources:
1. **ESO Logs Build Statistics** \u2014 aggregated data from real combat log parses showing what weapon combos, traits, and enchants top players actually use
2. **Knowledge Base** \u2014 curated reference material about ESO game mechanics, traits, enchants, and build theory

Always ground your answers in the data provided below. If the data doesn't cover the user's question, say so honestly.`;
var ANTI_HALLUCINATION = `## Important Rules
- ONLY reference data explicitly provided in the sections above
- If asked about something not covered by your data, say "I don't have data on that yet" rather than guessing
- Never invent parse scores, usage percentages, or gear recommendations not grounded in the provided stats
- When citing build statistics, mention the sample size (usage_count) so users understand confidence levels
- Keep answers concise and actionable \u2014 ESO players want practical advice, not essays
- Use ESO terminology naturally (e.g. "front bar", "back bar", "parse", "weave")
- Format responses with markdown for readability`;
var formatBuildStats = /* @__PURE__ */ __name((stats) => {
  const lines = stats.map(
    (s) => `- ${s.weapon_combo} | ${s.role} ${s.class} | Front: ${s.front_bar_trait}/${s.front_bar_enchant} Back: ${s.back_bar_trait}/${s.back_bar_enchant} | Avg parse: ${s.avg_parse_score.toLocaleString()} | Used by ${s.usage_count} players | Patch ${s.patch_version}`
  );
  return `## ESO Logs Build Statistics
The following are real aggregated build stats from combat log parses:
${lines.join("\n")}`;
}, "formatBuildStats");
var formatKnowledgeDocs = /* @__PURE__ */ __name((docs) => {
  const lines = docs.map((d) => `### ${d.title} (${d.doc_type})
${d.content}`);
...
chatRoute.post("/eso-chat", async (c) => {
  ...
  const [buildStats, embedding] = await Promise.all([
    queryBuildStats(c.env.DB, intent),
    c.env.AI.run(EMBEDDING_MODEL, { text: [body.message] })
  ]);
  const vectorResults = await c.env.VECTOR_INDEX.query(
    embedding.data[0],
    { topK: VECTORIZE_TOP_K, returnMetadata: "all" }
  );
  const vectorizeIds = vectorResults.matches.filter((m) => m.score > 0.5).map((m) => m.id);
  const knowledgeDocs = await queryKnowledgeDocsById(c.env.DB, vectorizeIds);
  ...
  const sources = {
    ...
    knowledgeDocs: vectorResults.matches.filter((m) => m.score > 0.5).map(
      (m) => ({
        title: m.metadata?.title ?? m.id,
        docType: m.metadata?.doc_type ?? "unknown",
        score: m.score
      })
    )
  };
  ...
  return streamSSE(c, async (stream2) => {
    let tokenId = 0;
    try {
      const aiStream = await c.env.AI.run(CHAT_MODEL, {
        messages: [
...
// src/routes/seed-vectorize.ts
var seedVectorizeRoute = new Hono2();
...
app.use(
  "*",
  cors({
    origin: ["http://localhost:3000", "http://localhost:3001", "https://esotk.com"],
