...
const requestedFullBaseUrl = process.env.FULL_BASE_URL ?? process.env.BASE_URL;
const rawFullPort = process.env.FULL_PORT ?? process.env.PORT ?? '3008';
const parsedFullPort = Number.parseInt(rawFullPort, 10);
const fullPort = Number.isFinite(parsedFullPort) ? parsedFullPort : 3008;
const fullBaseUrl = requestedFullBaseUrl ?? `http://localhost:${fullPort}`;
...
export default defineConfig({
  ...
  testIgnore: [
    ...
    '**/parse-analysis.smoke.spec.ts', // Covered by the dedicated smoke suite
  ...
  webServer: requestedFullBaseUrl
    ? undefined
    : {
        command: 'npm run dev',
        url: fullBaseUrl,
        env: {
          PORT: fullPort.toString(),
        },
